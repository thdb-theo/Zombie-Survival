...
class Window(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.quit_shortcut = QtGui.QShortcut(
    ...
    def home(self):
        ...
        self.not_log.move(3 * Window.width // 4 -
                          self.not_log.width() // 2, 50)
        if self.is_not_log:
            self.not_log.toggle()
        ...
        self.male_b = QtGui.QCheckBox(get_text("male"), self)
        self.male_b.move(Window.width // 4 - self.male_b.width() // 2, 120)

        self.female_b = QtGui.QCheckBox(get_text("female"), self)
        self.female_b.move(3 * Window.width // 4 -
                           self.female_b.width() // 2, 120)
        if self.gender == "m":
            self.male_b.toggle()
        else:
        ...
        self.nor_b = QtGui.QCheckBox(get_text("norwegian"), self)
        self.nor_b.adjustSize()
        self.nor_b.move(Window.width // 4 - self.male_b.width() // 2, 155)

        self.eng_b = QtGui.QCheckBox(get_text("english"), self)
        self.eng_b.adjustSize()
        self.eng_b.move(3 * Window.width // 4 -
                        self.female_b.width() // 2, 155)
        if self.lang == "norsk":
            self.nor_b.toggle()
        else:
            self.eng_b.toggle()
        ...
        self.fps_value_text.move(
            self.fps_slider.pos().x() + self.fps * 1.15 - 12, 240)
        ...
        def map_filter(m): return m.endswith(".txt")
        options = list(filter(map_filter, os.listdir("assets/Maps/")))
        self.map_dropdown = QtGui.QComboBox(self)
        self.map_dropdown.addItems(options)
        self.map_dropdown.setCurrentIndex(options.index(self.map))
        ...
        self.volume_value_text.move(
            self.volume_slider.pos().x() + self.volume * 200 - 12, 300)
        ...
        self.tile_length_value_text.move(
            self.tile_length_slider.pos().x() + self.tile_length * 2.35 - 27, 360)
        ...
        self.loopcolourtext.move(
            Window.width // 2 - self.fillcolour.width() // 2 + self.fillcolour.width(), 470)
        ...
        self.loopcolour.move(self.loopcolourtext.pos().x() +
                             self.loopcolourtext.width() + 15, 470)
    ...
    def volume_change(self):
        ...
        self.volume_value_text.move(
            self.volume_slider.pos().x() + self.volume * 2 - 12, 300)
    ...
    def tile_length_change(self):
        ...
        self.tile_length_value_text.move(
            self.tile_length_slider.pos().x() +
            self.tile_length * 2.35 - 27, 360
        )
    ...
    def commit(self):
        if (Options.mapname != self.map or
                Options.tile_length != self.tile_length or
                self.lang != Options.language):
            self.changed = True
