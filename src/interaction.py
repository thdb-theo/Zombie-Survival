...
def walking(survivor, keys):
    ...
    if keys[pygame.K_s]:  # South
        future_tile_num = survivor.get_number() + Options.line_length
        if future_tile_num <= Tile.amnt_tiles:
            future_tile = Tile.instances[future_tile_num]
            if future_tile.walkable:
                survivor.set_target(future_tile)
    ...
    if keys[pygame.K_a]:  # West
        ...
        if future_tile_num % Options.line_length != Options.line_length - 1:
            future_tile = Tile.instances[future_tile_num]
            if future_tile.walkable:
                survivor.set_target(future_tile)
                survivor.rotate(pi)
    ...
    if keys[pygame.K_d]:  # East
        future_tile_num = survivor.get_number() + 1
        if future_tile_num % Options.line_length != 0:
            future_tile = Tile.instances[future_tile_num]
            if future_tile.walkable:
                survivor.set_target(future_tile)
                survivor.rotate(0)
...
def shooting(survivor, keys):
    ...
    if keys[pygame.K_LEFT]:
        survivor.rotate(pi)
        Bullet(survivor.centre, Vector(-Options.bullet_vel, 0), survivor.current_gun)
    elif keys[pygame.K_RIGHT]:
        survivor.rotate(0)
        Bullet(survivor.centre, Vector(Options.bullet_vel, 0), survivor.current_gun)
    elif keys[pygame.K_UP]:
        survivor.rotate(pi / 2)
        Bullet(survivor.centre, Vector(0, -Options.bullet_vel), survivor.current_gun)
